    ...
    internal class SubjectRepository : ISubjectRepository
        {
            public void AddSubject(Subject subject)
            {
                ...
                catch (SQLiteException ex)
                {
                    ...
                    {
                        throw new Exception($"A subject with the name '{subject.SubjectName}' already exists for Course ID {subject.CourseId}.", ex);
                    }
            ...
            public Subject GetSubjectByNameAndCourse(string subjectName, int courseId)
            {
                try
                {
                    ...
                    {
                        var cmd = connection.CreateCommand();
                        cmd.CommandText = "SELECT SubjectId, SubjectName, CourseId FROM Subjects WHERE SubjectName = @SubjectName AND CourseId = @CourseId";
            ...
            public List<Subject> GetAllSubjectsWithCourseNames()
{
    ...
    using (var connection = DatabaseManager.GetConnection())
    {
        ...
        cmd.CommandText = @"
            SELECT 
                s.SubjectId, s.SubjectName, s.CourseId,
                c.CourseName
            FROM Subjects s
